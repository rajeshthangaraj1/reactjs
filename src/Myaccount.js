...
class Myaccount extends React.Component { 
	...
	handleFormSubmit = () => { 
		...
		params.append('customer_first_name', formPayload.firstname); 
		params.append('customer_last_name', formPayload.lastname); 
		params.append('customer_email', formPayload.email); 
		params.append('customer_phone', formPayload.mobile); 
		params.append('customer_birthdate', formPayload.birthday); 
		params.append('customer_postal_code', formPayload.postal); 
		params.append('customer_address_name', formPayload.address); 
		params.append('customer_company_name', formPayload.company); 
		params.append('customer_address_line1', formPayload.unitnumber1); 
		params.append('customer_address_line2', formPayload.unitnumber2); 
		params.append('customer_id', cookie.load('UserId')); 
		this.setStateLoading('Loading');
		
		axios.post(APIURL+"customer/updateprofile", params)
		.then(response => {
				...
				setTimeout(function() {
					if(response.data.status==="ok"){
						...
						params.append('app_id', APPID); 
						params.append('type', 'web'); 
						params.append('secondary_address_id',  cookie.load('UserSecAddId'));
						params.append('refrence', cookie.load('UserId')); 
						params.append('customer_email', formPayload.email); 
						params.append('customer_first_name', formPayload.firstname); 
						params.append('customer_last_name', formPayload.lastname); 
						params.append('customer_phone', formPayload.mobile); 
						params.append('customer_address_name', formPayload.unitnumber1); 
						params.append('customer_address_name2', formPayload.unitnumber2); 
						params.append('customer_address_line1', formPayload.address); 
						params.append('customer_postal_code', formPayload.postal); 
						params.append('customer_status', 'A'); 	
						axios.post(APIURL+"customer/secondary_address_edit", params).then(response => {
...
class Form extends React.Component {
	...
	loadSecAddr(dataArr){
		...
		var htmlStr = dataArr.reverse().map(
				function(data, index) { 
				if(index===0){
					cookie.save("UserSecAddId",data.secondary_address_id); 
					return  <div key={index}><p><b>Default Billing Address</b></p>
								   
								  <p id="user-default-address" > 
				<img src={"/img/icons/modal-close.png"} alt="" className="settings-img" style={{width:'15px',float:'right'}} onClick={() => {this.DeleteSecAddr(data.secondary_address_id)}} />
				{data.address}
				{ data.unit_code ? <br /> :''}
	...
	DeleteSecAddr(pmId){
		...
		params.append('app_id', APPID); 
		params.append('type', 'web'); 
		params.append('secondary_address_id',  pmId);
		params.append('refrence', cookie.load('UserId')); 
			
		axios.post(APIURL+"customer/secondary_address_remove", params)
			.then(response => { 
				this.setStateLoading('ok');
				this.getSecAddr();
		  })
	...
	saveNewAddress(){
		...
		if(vsuccess){
			...
			params.append('app_id', APPID); 
			params.append('type', 'web'); 
			params.append('customer_first_name', cookie.load('UserFname')); 
			params.append('customer_last_name', cookie.load('UserLname')); 
			params.append('customer_email', cookie.load('UserEmail'));  
			params.append('customer_address_line1', document.getElementById("customer_address_line1").value); 
			params.append('customer_postal_code', document.getElementById("customer_postal_code").value); 
			params.append('customer_address_name', document.getElementById("customer_address_name").value);
			params.append('customer_address_name2', document.getElementById("customer_address_name2").value);
			params.append('refrence', cookie.load('UserId')); 
			
			axios.post(APIURL+"customer/secondary_address_add", params)
			.then(response => { 
				if(response.data.status==="ok"){ 
					this.setStateLoading('ok');
